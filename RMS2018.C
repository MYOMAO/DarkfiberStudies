...
void RMS2018()
{
	...
	Inter->SetTitle("");
	c1->SaveAs(Form("Results/%d/%s/ProjectionRange.pdf",year,ordertag.Data()));
	...
	TH1D * hProjX = new TH1D("hProjX","hProjX",XHisBins,XBL,XBR);
	TH1D * hProjY = new TH1D("hProjY","hProjY",YHisBins,YBL,YBR);
	...
	c1->SaveAs(Form("Results/%d/ProjXHis-%s.pdf",year,ordertag.Data()));
	hProjY->Draw();
	c1->SaveAs(Form("Results/%d/ProjYHis-%s.png",year,ordertag.Data()));
	c1->SaveAs(Form("Results/%d/ProjYHis-%s.pdf",year,ordertag.Data()));
	...
	MEHis->Draw();
	c2->Update();
	c2->SaveAs(Form("Results/CentralProj/%d%sRMSPlot.png",year,ordertag.Data()));
