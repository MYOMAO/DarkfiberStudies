...
void MakeFile(){
	...
	if(recab ==1){
		if(PosFix ==1){
			...
			t->AddFriend(t2);
			TH3D *Energyhis = new TH3D("EnerTMath::Sin(theta)gyhis","",XBins, Xmin,Xmax,YBins,Ymin,Ymax,EBins,Emin,Emax);
	...
	TH3D *Energyhis = new TH3D("Energyhis","",XBins, Xmin,Xmax,YBins,Ymin,Ymax,EBins,Emin,Emax);
	t->Draw(Form("sum_energy_T*%s:truth_y * 10 + 5*info.hodo_v*%d + %f:truth_z * %f * 10 - 5*info.hodo_h*%d + %f>>Energyhis",scale.Data(),dohodo,shift,anglecorr,dohodo,shift));
